...
1. 修改空中防止连跳过头的逻辑, 将原先<b>缩放新的方向向量长度为 RunTopSpeed + g_cvBhopImpulse.FloatValue</b>更改为<b>直接缩放新的方向向量长度为当前速度大小</b>达到仅改变速度方向而不改变速度大小的目的, 由于 RunTopSpeed 为 TankSpeed, 当连跳速度大于 RunTopSpeed + 加速度时方向修正会造成减速效果<br>
2. 修复无视野连跳时仅按前进方向键 fwdOnly 情况 vFwd = vDir (无视野时 vDir 为当前速度向量角度, 有视野时 vDir 为到目标方向的方向向量) 导致 vFwd 前进方向向量被覆盖而无法加速的问题
</details>